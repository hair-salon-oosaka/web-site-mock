...
| ファイル | 内容 | 状態 |
|---|---|---|
| index.html | TOP（ヒーロー・コンセプト・メニュー概要・ギャラリー・スタイリスト・営業カレンダー・アクセス・予約CTA） | ✅ 完成 |
| menu.html | メニュー・料金詳細（スティッキーカテゴリナビ・クーポンバッジ付き全メニュー一覧） | ✅ 完成 |
| reservation.html | Web予約フォーム（日程→時間→メニュー→情報入力→確認→完了） | ✅ 完成 |

## index.html セクション構成（順番）
1. ヒーロー（ロゴ `noble. 野江` + キャッチコピー + CTAボタン）
2. コンセプト
3. メニュー概要
4. ギャラリー
5. スタイリスト
6. 営業日カレンダー（`#calendar`）― 麻色背景、営業=苔色丸・休業=赤茶丸、2ヶ月表示
7. アクセス（`#access`）― 左:住所情報、右:Google Maps埋め込み
8. 予約CTA

## ロゴ仕様（index.html）
- ヒーロー時：`noble.`（游明朝体 Light italic）+ `野江`（游明朝体 Regular）横並び、白文字
- スクロール後（stuck）：logo.png 画像に切り替え
- クラス構成：`.logo-en`（noble.）/ `.logo-ja`（野江）/ セパレーターなし

## 営業カレンダー仕様（モック）
- セクションID：`#calendar`、クラス：`.business-calendar`
- 休業日はJSの `closedDates` 配列（`YYYY-MM-DD`形式）で管理
- 当月・翌月の2ヶ月を表示（PC横並び・モバイル縦積み）
- 営業日：苔色 `#6B7B5E` 丸背景・白文字 / 休業日：赤茶 `#B94A48` 丸背景・白文字
- 今日：ブラウン枠線リング
- 営業時間ボックス：10:00〜20:00（最終受付19:00）を表示
- Phase 2でSupabase `closed_dates` テーブルから動的取得予定

## メニュー構成（menu.html）
- カテゴリ6つ：クーポン・セット / カット / カラー / パーマ・縮毛矯正 / TR・ヘッドスパ / その他
- バッジ3種：新規限定（苔色）/ 全員対象（テラコッタ）/ 再来限定（ブラウン）
- クーポンカードは左ボーダーカラーでバッジ種別を識別
...
## フェーズ
- ✅ Phase 0：デザインモック（HTML）＋仮予約フォーム ← **完了（2026-04）**
  - index.html / menu.html / reservation.html 完成
  - ロゴ・カラー・フォント・レイアウト確定
  - メニュー構成・バッジシステム確定
  - 営業日カレンダーUI実装（静的）
  - DB設計（メニュー・休業日・予約・ブロック枠）草案完了
